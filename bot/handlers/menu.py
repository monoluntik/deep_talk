...
async def _show_menu(chat_id: int, bot: Bot) -> None:
    ...
    if not categories:
        await bot.send_message(
            chat_id,
            "Пока нет категорий.\nИспользуйте /generate <тема> чтобы создать первую!"
        )
        return
...
# Выбор категории — только cat:{int}, не cat:delete:... или cat:extend:...
...
async def handle_category_select(callback: CallbackQuery, bot: Bot) -> None:
    ...
    if not q:
        ...
        await callback.message.edit_text(
            ...
            "Используйте /generate <тема> чтобы создать новые вопросы,\n"
